...
public class UnmanageVMManagerImpl implements UnmanageVMManager {
     ...
     * - VM must not have any VM snapshots
    ...
    private void performUnmanageVMInstancePrechecks(VMInstanceVO vmVO) {
        if (hasVMSnapshotsPriorToUnmanageVM(vmVO)) {
            throw new UnsupportedServiceException("Cannot unmanage VM with id = " + vmVO.getUuid() +
                    " as it has VM snapshots. Please remove VM snapshots before unmanaging.");
        }
    ...
    private boolean hasVMSnapshotsPriorToUnmanageVM(VMInstanceVO vmVO) {
        List<VMSnapshotVO> vmSnapshots = vmSnapshotDao.findByVm(vmVO.getId());
        return CollectionUtils.isNotEmpty(vmSnapshots);
    }
    ...
    public boolean unmanageVMInstance(long vmId) {
        ...
        if (vmVO == null || vmVO.getRemoved() != null) {
            throw new InvalidParameterValueException("Could not find VM with id = " + vmId);
